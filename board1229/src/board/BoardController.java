...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(url.indexOf("list.do") != -1) {
			...
			// 페이지 나누기를 위한 처리
			int curPage = 1;
			// 숫자 처리는 null 포인트 익셉션이 잘 일어나기 때문에 if문 처리해주는게 좋다.
			...
			//(request,파일업로드 디렉토리,업로드용량,파일인코딩,중복파일정책)
			...
			// 체크박스에 체크가 되었으면, value없이 썼기 때문에 on이 디폴트가 됨
			...
			// 파일업로드를 안쓰더라도 MutipartRequest 처리를 해야한다.
			MultipartRequest multi = new MultipartRequest(request, Constants.UPLOAD_PATH, Constants.MAX_UPLOAD, "utf-8", new DefaultFileRenamePolicy());
			// 삭제할 게시물 번호
			int num = Integer.parseInt(multi.getParameter("num"));
			dao.delete(num);
