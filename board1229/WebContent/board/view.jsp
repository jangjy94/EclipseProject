...
// url에서 comment_list.do 서블렛에서 list.do로 인식될 수 있다.
function comment_list() {
	// 백그라운드로 서버 호출
	// url 이름 정할때 서블릿에서 처리하는 이름을 염두해 두고 네이밍 할것
	// ex) url에서 comment_list.do 하면 자칫 버블릿에서 list.do로 알고 처리될 수 있음
	$.ajax({
		type : "post",
		url : "${path}/board_servlet/commentList.do",
		data : "num=${dto.num}",
		success : function(result) {
			// result변수는 responseText 서버의 응답 테스트
